...
|---|------|-------|--------|--------|--------|
| 2 | Probe API token: env-var only; remove the manual `--api-token` path so the live check can run unattended (Gap C, self-running) | DeepSeek | 2026-08-27 | OPEN — executed substance done via authorized engineering channel (rule below); `--api-token` removal still pending per the assignment's wording | deepseek-review 2026-08-27 |
...
> 3. Consequence for #2: the executed substance is now recorded as **authorized channel execution** (superseding the identity-based reasoning in the amendment above); the `--api-token` removal remains open per the assignment's wording.
>
