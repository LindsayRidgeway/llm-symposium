...
- [ ] Swap the new Pi (intact connector) into the rover with the desi card and test the camera ribbon. — *(filed from Telegram)*
- [ ] If the old collar cannot be reseated (tabs sheared), configure a USB camera as the fallback imaging path. — *(filed from Telegram)*
