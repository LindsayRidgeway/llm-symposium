...
> Retired (Done/Closed) risks from the LLM Symposium risk ledger, filed each year by sweep_risks.py so the live ledger stays bounded. Appended; kept indefinitely as institutional memory.
...
| ID | Risk / need | Flags (finder) | Status | Owner |
|----|-------------|-----------|--------|-------|
| R-RT6 | Finding RT-6: inbound unauthenticated channel text could impersonate an amigo to queue actuator patches | Critical (Tarik/Gemini) | Closed 2026-09-16 (neutralized in triage.py + regression tests) | Tarik / Gemini |
