...
# ...and "wholly" means wholly: every word of the shorter form must appear in the longer one.
# A bare similarity is not enough. Measured 2026-09-25: "install the new pi card and verify the
# camera works again" is 9 of 10 words of "...works end to end" (containment 0.90) and the two are
# different tasks — one is a re-do, one is a first pass. At 0.90 the module silently merged them;
# the test for it was written first and is the reason the boundary is stated as a constant.
...
def same_item(a: str, b: str, threshold: float = NEAR_DUPLICATE) -> bool:
    ...
    if similarity(a, b) >= threshold:
        return True
    ta, tb = set(na.split()), set(nb.split())
    if (min(len(ta), len(tb)) >= MIN_CONTAINMENT_TOKENS
            and containment(a, b) >= CONTAINMENT_REQUIRED):
        return True
    return False
