...
- **Automated test pipeline (recommended):** to prevent future regressions, add a GitHub Actions workflow that runs `python3 tests/test_projection.py` on every push and scheduled run. A documentation-only fix without code changes must not mark these issues as closed.
