...
| C — layer attribution unverified | `--api-token` mode calls the TickTick Open API directly for comparison |
...
# Probe with live TickTick API check (needs an OAuth access token):
python3 probes/ticktick_recurrence_probe.py --api-token YOUR_TOKEN
```
