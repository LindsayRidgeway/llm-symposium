...
Usage:
    python3 probes/ticktick_recurrence_probe.py [fixture.json]

The Gap C layer-isolation check (direct TickTick Open API call vs. connector
behavior) runs when a token is available. **Environment-variable only**
(assignment #2 / workaround protocol): a CLI token would leak via shell
...
under the canonical `TICKTICK_API_TOKEN` name. Without any token, layer
...
def check_live_api(token: str) -> dict:
    ...
    # An empty-body task query returns HTTP 200 with an empty list, so scope
    # the query to a real project: take the first project id from the projects
    # response (the account has 7 projects). Short body samples are kept for
    # the report so the next iteration can read what the API actually returns.
    project_id = None
    try:
...
def run(fixture_path: str, api_token: str | None = None) -> str:
    ...
    if api_token:
        ...
        lines.append("No TickTick token provided (env `TICKTICK_API_TOKEN`/`TICKTICK_API_KEY` — "
                     "in CI the repository secret `TICKTICK_API_KEY` is wired in automatically). "
                     "Direct API isolation test **not run**; layer attribution remains unverified.")
    lines.append("")
...
def main() -> int:
    ...
    # Env-var only (assignment #2 / workaround protocol): a CLI token can
    # leak via shell history and process listings. Accept both the canonical
    # name and the repository-secret name.
    token = os.environ.get("TICKTICK_API_TOKEN") or os.environ.get("TICKTICK_API_KEY")
