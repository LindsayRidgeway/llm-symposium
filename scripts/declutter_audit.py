...
def find_drift(files, blobs):
    ...
    * A generator is re-run in the tree, so this detector mutates as it measures. The
      ...
      **Restored means every file the generator touched, not the one that named it.**
      Added 2026-09-26: `gen_index.py` rewrites every index, so re-running it for
      `scripts/README.md` also rewrote `discussions/README.md`; and `compile_agenda.py`
      is named by its own output, so re-running it for `scripts/compile_agenda.py`
      rewrote `channels/agenda.md`. The first version put back only the file whose turn
      had come — and because a generator is idempotent, by the time the sibling's own
      turn came its bytes already matched the generator, so it was neither reported as
      drift nor put back. The real-repo test `test_it_does_not_mutate_the_tree` failed on
      `main` for this reason, with `M channels/agenda.md, discussions/README.md,
      scripts/README.md` left behind. The whole tree is now restored against the pristine
      snapshot (`blobs`) after every generator run, so each file is measured clean.
    * It must never re-run *itself*. The report declares "GENERATED by
    ...
    for f in files:
        ...
        try:
            ...
            # Restore against the pristine snapshot, not this file's bytes at its own turn: a
            # generator writes its siblings too (see the docstring), and once a sibling has been
            # rewritten its own turn can no longer see the difference. `drifted` records whether
            # *this* file is one the generator disagrees with; every other file is only put back.
            drifted = False
            for g in files:
                current = read(g)
                if current != blobs.get(g, current):
                    if g == f:
                        drifted = True
                    with open(os.path.join(REPO, g), "wb") as fh:
                        fh.write(blobs[g])
            if drifted:
                out.append((f, gen))
